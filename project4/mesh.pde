/**
 * @file mesh.pde 
 * @brief Corner table for triangle meshes
 * @author J. Rossignac
 */

String [] fn= {"HeartReallyReduced.vts","horse.vts","bunny.vts","torus.vts","flat.vts",
"tet.vts","fandisk.vts","squirrel.vts","venus.vts","mesh.vts","hole.vts","gs_dimples_bumps.vts"};
...
class Mesh {
 ...
 // showEdges shoes edges as gaps. Smooth shading works only when !showEdge 
 Boolean showEdges=false; 
 Boolean showCurvature=false;  

  // Hamiltonian variables
  int c_seed;
  int t_seed;
  Boolean[] v_m = new Boolean[maxnv]; 
  Boolean[] t_m = new Boolean[maxnt];    

  int numPathTriangles;
  int pathTriangles[];
  Curve pathCurve;
  int numT0; int numT1; int numT2;
  int Ti[];
  Boolean inACurve[];
...
//  ==================================== INIT, CREATE, COPY ====================================
 ...
 /**
  * @function declareVectors
  * @brief Init vertices and normals and init triangle normals and skeleton labels
  */
 Mesh declareVectors() {
   for (int i=0; i<maxnv; i++) {G[i]=P(); Nv[i]=V();};   
   for (int i=0; i<maxnt; i++) Nt[i]=V();
...
void makeDChain (float w, int m) { // make a chain of size 2w wiht m elements
  ...
  * @brief  Reset the seed and current corner and the markers for corners, triangles, and vertices
  */
 Mesh resetMarkers() { 
   ...
   // Important initialize markers too!
   for (int i=0; i<maxnt; i++) { t_m[i]=false; }
   for (int i=0; i<maxnv; i++) { v_m[i]=false; }
 ...
 /**
  * @function updateON
  * @brief Recomputes O and normals
  */ 
 Mesh updateON() {
   computeO(); 
   normals(); 
 ...
 /**
  * @function ring_expander
  */
 void ring_expander() {
  println("Start ring_expander"); 
  // Reset markers to false
  for( int i = 0; i < maxnv; ++i ) { v_m[i] = false; }  
  for( int i = 0; i < maxnt; ++i ) { t_m[i] = false; }     
  ...
  // Mark vertices of initial seed triangle as visited
  v_m[ v(n(c)) ] = true;
  v_m[ v(p(c)) ] = true;
  
  pathTriangles = new int[maxnt];

  do {
    // If triangle not invaded yet, invade it
    if( v_m[ v(c) ] == false ) {
      v_m[ v(c) ] = true;
      t_m[ t(c) ] = true;
      pathTriangles[numPathTriangles] = t(c);
      numPathTriangles++;
    }
    // If already invaded, go back one triangle (but now try the other side)
    else if( t_m[ t(c) ] == false ) {
      c = o(c);
    }
    
    // Go to the right
    c = r(c);
  }while( c != o(c_seed) ); // Until the beginning
  
  println("Finished ring_expander");
  
 } 

/**
 * @function getTrianglesType
 * @brief Get num T0, T1 and T2
 */
void getTrianglesType() {

   Ti = new int[maxnt];
   T0 = new int[maxnt]; T1 = new int[maxnt]; T2 = new int[maxnt];
   numT0 = 0; numT1 = 0; numT2 = 0;
   inACurve = new Boolean[maxnt];  
   ...
   for( int i = 0; i < numPathTriangles; ++i ) {
     
     ti = pathTriangles[i]; ci = c(ti);
     ...
     if( count == 0 ) { T0[numT0] = i;numT0++; }
     else if( count == 1 ) { T1[numT1] = i; numT1++; }
     else if( count == 2 ) { T2[numT2] = i; numT2++; }

     Ti[i] = count;
   } // end for

   println("Num of T0: " + numT0 + " T1: " + numT1 + " T2: " + numT2 + " total triangles in path: " + numPathTriangles );
  ...
  * @function getPath
  */
 void getPath( int _index ) {
   ArrayList<pt> path = new ArrayList<pt>();
 ...
 /**
  * @function getPathFromT0
  */
 void getPathFromT0( int _index ) {
 
  int ti; int ci;
  int ta1, ta2, ta3;
  int waypoint = -1;
  
  ti = pathTriangles[_index]; ci = c(ti);    
  // Its 3 neighbors    
  ta1 = t( o(ci) ); 
  ta2 = t( o(n(ci)) );
  ta3 = t( o(p(ci)) );
  if( t_m[ta1] == false || t_m[ta2] == false || t_m[ta3] == false ) { println("Something is wrong with this "); }

  if( inACurve[ta1] == false ) { waypoint = ta1; }
  else if( inACurve[ta2] == false ) { waypoint = ta2; }
  else if( inACurve[ta3] == false ) { waypoint = ta3; }
  else{ println("Something wrong in picking the waypoint "); }

  // Find the location of the noOnCurve triangle
  int ind = 0;  
  for( int j = 0; j < numPathTriangles; ++j ) {
    if( pathTriangles[j] == waypoint ) { ind = j; break; }
  } 
  // Start path
  ArrayList<pt> path = new ArrayList<pt>();
  int i;
  ...
  // Add the initial T0
  ti = pathTriangles[_index];
  ci = c(ti);
  path.add( P( g(ci), g(ci+1), g(ci+2) ) );   
   ...
   while( Ti[i] != 2 ){ 
     ...
     path.add( P( g(ci), g(ci+1), g(ci+2) ) );
     i++;
     if( i == numPathTriangles ) {break;}
   }
   
   // Add last
   if( i < numPathTriangles ) {
     ti = pathTriangles[i];
     inACurve[ti] = true; 
     ci = c(ti);
     path.add( P( g(ci), g(ci+1), g(ci+2) ) );
 ...
 void getPathCurve() {
   ...
   // 0. Set up
   numCurves = 0;
   curvesPts = new ArrayList< ArrayList<pt> >();
   pathCurves = new ArrayList<Curve>();
   for( int i = 0; i < maxnt; ++i ) { inACurve[i] = false; }
   
   // 1. Get first curve (from the seed)
   getPath( 0 );  
  
   // 2. Get curves from T0 
   for( int i = 0; i < numT0; i++ ) {
     getPathFromT0( T0[i] );
   }
   ...
   // Add curves
   for( int i = 0; i < curvesPts.size(); ++i ) {

     Curve c; 
     c = new Curve( (curvesPts.get(i)).size() );
     c.declarePoints();

     // Load points
     for( int j = 0; j < c.n; ++j ) {
       c.P[j].set( ( curvesPts.get(i) ).get(j) );
     }
  ...
  println("Finish path curves processing ");
}
 
 /**
  * @function showPathCurve
  */
 void showPathCurve() {
...
// ============================================= MESH MANIPULATION =======================================
  ...
  /**
   * @function pickc
   * @brief Picks closest corner to X
   */
  void pickc (pt X) {
    for (int b=0; b<nc; b++) {
      if( vis[tm[t(b)]] ) {
        if(d(X,cg(b))<d(X,cg(cc))) {
          cc=b; pc=b; 
        }
      }
    } 
  ...
  /**
   * @function pick_seed
   * @brief Picks closest corner to X and fix it as seed
   */
  void pickc_seed (pt X) {
    c_seed = 0;
    int c_pseed = 0;
    
    for ( int b = 0; b < nc; b++ ) {
      if( vis(b) ) {
        if(d(X,cg(b))<d(X,cg(c_seed))) {
          c_seed = b; 
          c_pseed=b; 
        }
      }
...
// ============================================= Bounding box and detail size =======================================
  ...
  /**
   * @function computeBox
   * @brief Enclosing box - Computes center Cbox and half-diagonal Rbox of minimax box
   */
  Mesh computeBox() { 
...
Mesh loadMeshVTS() {
  ...
  println("After load");
  if (loadPath == null) {
    println("No input file was selected..."); return this;
  }
